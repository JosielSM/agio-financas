(() => {
  ...
  const toLoanRow = (loan, ownerId) => ({
    ...
    late_fee: loan.lateFee || 0, total: loan.total, installment: loan.installment, due_date: loan.dueDate,
    payment_states: loan.paymentStates || {}, custom_dates: loan.customDates || {}, archived: Boolean(loan.archived),
    created_at: loan.createdAt
  });
  const fromLoanRow = row => ({
    id: row.id, contract: row.contract, clientId: row.client_id, amount: Number(row.amount), rate: Number(row.rate),
    installments: row.installments, frequency: row.frequency, lateFee: Number(row.late_fee || 0), total: Number(row.total),
    installment: Number(row.installment), dueDate: row.due_date, paymentStates: row.payment_states || {},
    customDates: row.custom_dates || {}, archived: row.archived, createdAt: row.created_at
  });
  window.credmaisBridge = {
    ...
    async load() {
      ...
      const [clientsResult, loansResult] = await Promise.all([client.from('clients').select('*').order('created_at'), client.from('loans').select('*').order('created_at')]);
      if (clientsResult.error) throw clientsResult.error;
      if (loansResult.error) throw loansResult.error;
      return { clients: clientsResult.data.map(row => ({ id: row.id, name: row.name, cpf: row.cpf, phone: row.phone, email: row.email || '', note: row.note || '', blacklisted: row.blacklisted })), loans: loansResult.data.map(fromLoanRow) };
    },
    async sync(user, clients, loans) {
      if (!client || !user?.id) return;
      const clientRows = clients.map(item => ({ ...item, owner_id: user.id }));
      const loanRows = loans.map(item => toLoanRow(item, user.id));
      if (clientRows.length) { const { error } = await client.from('clients').upsert(clientRows); if (error) throw error; }
      if (loanRows.length) { const { error } = await client.from('loans').upsert(loanRows); if (error) throw error; }
    }
  };
})();
